...
def imgcon(pdf_root,out_root,model_root):
    ...
    ### IMAGE BYTES [Memory size checking]
    ...
    def img_read(img_root,img_cnt):
        ...
        for i in range(img_cnt):
            root=os.path.join(img_root, str(i) + '.png')
            rgb=cv2.imread(root)
            ...
            ## IMAGE FILE SIZE CHECK LIST IF NEEDED TO USE 
    ...
    def pdimg(pdf_root,out_root):
        ...
        for i in range(len(images)):
            # images[i].save('page'+ str(i) +'.jpg', 'JPEG')
            pth2=os.path.join(out_root, str(i) + '.png')
            images[i].save(pth2)
            image=images[i]
        return i
    ...
    root=os.path.join(out_root, str(img_cnt) + '.png')
    # print(img_cnt)
                        ##     ### Predict
    ...
    prd=bank.logo_predict(model_root,root)
...
out_root=r'/home/selva/Selva/Products/Task/Datas/ihritik'
pdf_root=r'/home/selva/Selva/Products/Task/Datas/ax1-converted.pdf'
model_root=r'/home/selva/Selva/Products/Task/Datas/model/logomdl1.h5'
prd=imgcon(pdf_root,out_root,model_root)
print(prd)
